...
**Decision rule — body rendering (see Risk R1):** First try rendering the `fields.mdx` output from the Reader API. If runtime MDX-string rendering proves brittle within the build, **switch the three body fields to `fields.markdoc`** and render with `@markdoc/markdoc` (clean programmatic transform→render). If *that* is still awkward, fall back to **per-language Astro content-collection entries** (`src/content/posts/<lang>/<slug>.mdx` via the `glob` loader + native `render()`), keeping slugs synced. Pick the first option that renders reliably; document the choice at the top of the routing file.
